...
export default function RelatorioCompra({ linhas }: { linhas: MovimentacaoRelatorio[] }) {
  if (linhas.length === 0) {
    return (
      <div className="rounded-card border border-dashed border-border bg-surface px-6 py-12 text-center">
        <p className="font-semibold text-text-primary">Nenhuma compra no período</p>
        <p className="mx-auto mt-1.5 max-w-sm text-sm text-text-secondary">
          Ajuste os filtros acima ou lance compras em Movimentações.
        </p>
      </div>
    )
  }
  ...
  const valorTotalInvestido = linhas.reduce((s, l) => s + (valorLiquido(l) ?? 0), 0)
  ...
  const porFornecedor = [...agruparPorChave(linhas, (l) => l.cliente?.nome ?? 'Não informado').entries()]
  ...
  const porCategoria = [...agruparPorChave(linhas, (l) => l.categoria?.nome ?? '—').entries()]
    .map(([nome, ls]) => ({ nome, quantidade: ls.reduce((s, l) => s + l.quantidade, 0) }))
    .sort((a, b) => b.quantidade - a.quantidade)

  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <KpiCard label="Cabeças compradas" value={formatQuantidade(total)} />
        <KpiCard label="Valor total investido" value={formatMoeda(valorTotalInvestido)} hint="líquido" />
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <div className="rounded-card border border-border bg-surface p-5">
          <h3 className="mb-3 text-sm font-semibold text-text-primary">Evolução do preço da arroba</h3>
        ...
        <div className="rounded-card border border-border bg-surface p-5">
          <h3 className="mb-3 text-sm font-semibold text-text-primary">Ranking de fornecedores (valor)</h3>
          <ResponsiveContainer width="100%" height={240}>
            <BarChart data={porFornecedor} layout="vertical" margin={{ left: 24 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#DDE4E1" />
              <XAxis type="number" tick={{ fontSize: 12 }} />
              <YAxis type="category" dataKey="nome" tick={{ fontSize: 12 }} width={100} />
              <Tooltip formatter={(v: any) => formatMoeda(v)} />
              <Bar dataKey="valor" radius={[0, 4, 4, 0]}>
                {porFornecedor.map((_, i) => (
